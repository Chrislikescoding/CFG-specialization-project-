# this function extracts and formats the hidtory data downloaded from Google maps
import json
import csv
from copy import deepcopy

f = open('Google_history_download_monthly.json', 'r')

# read the json

with open("Google_history_download_monthly.json", "r") as read_file:
...
for key, element in enumerate(access):
    ...
    try:
        ...
        # choose in passnger vehicle to eliminate leisure activities such as walking,cycling etc
        if activity_type == 'IN_PASSENGER_VEHICLE':
            data_to_csv.append(deepcopy(data_dict))
# loop through dictionary and work out the "to" place as it will actually always be in a the next row
...
fieldnames=[{'from place': 1, 'from postcode':  2, 'to place': 3,
             'distance':   4, 'time': 5, 'date': 6, 'activity type': 7}]
